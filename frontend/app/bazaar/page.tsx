...
export default function DivineBazaar() {
    ...
    fetch("/api/products")
      ...
      .then(data => {
        ...
        const mappedProducts = records.map((rec: Record<string, any>, idx: number) => ({
          ...
          quantity: isNaN(Number(rec.fields['Quantity'])) || rec.fields['Quantity'] === '' ? 0 : Number(rec.fields['Quantity']),
        }));
  ...
  const handleCheckoutSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        setPendingToast({ message: 'Order placed successfully!', type: 'success' });
        setCheckoutStep('thankyou');
        if (showCheckout && showCheckout.mode === 'cart') setCart({});
      } else {
        setPendingToast({ message: 'Failed to place order. Please try again.', type: 'error' });
      }
    } catch (err) {
      setPendingToast({ message: 'Failed to place order. Please try again.', type: 'error' });
    }
  ...
  return (
    <div className="relative min-h-screen bg-gradient-to-br from-blue-100 via-purple-100 to-white pb-16 overflow-hidden">
      {/* Toast */}
      {toast && toastVisible && (
        <div className={`fixed top-6 left-1/2 -translate-x-1/2 z-50 min-w-[220px] max-w-xs bg-white shadow-lg rounded-lg px-6 py-4 flex flex-col items-start border ${toast.type === 'success' ? 'border-green-400' : 'border-red-400'}`}>
          <span className={`font-semibold mb-2 ${toast.type === 'success' ? 'text-green-600' : 'text-red-600'}`}>{toast.type === 'success' ? 'Order Placed!' : 'Order Failed'}</span>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      ...
                      <div className="flex flex-col gap-2">
                        <input
                          ...
                          value={checkoutForm.name !== '' ? checkoutForm.name : (session?.user?.name || '')}
                        ...
                        <input
                          ...
                          placeholder="Email"
                          className="w-full border rounded-lg pl-4 pr-4 py-2 bg-[#13294b] text-blue-100 focus:ring-2 focus:ring-blue-400 focus:border-blue-400 transition"
                          value={checkoutForm.email !== '' ? checkoutForm.email : (session?.user?.email || '')}
            ...
            >
              ...
              <div className="flex mt-6 w-full">
                {modalProduct.quantity === 0 ? (
                  <button
                    className="w-full py-3 rounded-lg font-semibold text-lg shadow transition bg-gray-300 text-gray-400 cursor-not-allowed"
                    disabled
                  >
                    Out of Stock
                  </button>
                  ...
                  <>
                    <button
                      className={`w-full py-3 rounded-lg font-semibold text-lg shadow transition ${modalProduct.quantity === 0 ? 'bg-gray-300 text-gray-400 cursor-not-allowed' : 'bg-gradient-to-r from-blue-600 to-purple-600 text-white hover:opacity-90'}`}
                      onClick={(e) => { if (modalProduct.quantity === 0) return; e.stopPropagation(); handleAddToCart(modalProduct); setModalProduct(null); }}
                      disabled={modalProduct.quantity === 0}
                    >
                      {modalProduct.quantity === 0 ? 'Out of Stock' : 'Add to Cart'}
                    </button>
                  </>
                )}
              </div>
      ...
      <section className="max-w-5xl mx-auto px-4 pt-20 pb-14 text-center flex flex-col items-center">
        ...
        <div className="flex gap-6">
          ...
          <button
            className={`px-8 py-4 rounded-full font-medium transition-all duration-300 shadow-lg hover:shadow-xl transform hover:scale-105 text-lg flex items-center gap-2
              ${Object.keys(cart).length === 0
      ...
      <section id="products" className="w-full px-4 py-12 bg-gradient-to-br from-blue-50 via-white to-purple-50 min-h-screen">
        <div className="max-w-7xl mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
          {displayProducts.map((product: Product, idx: number) => {
            console.log('Product:', product.id, product.name, 'Quantity:', product.quantity);
            return (
              <motion.div
                key={product.id}
              ...
              >
                ...
                <div className="p-4 flex flex-col justify-between flex-1">
                  ...
                  <div className="mt-auto">
                    {product.quantity === 0 ? (
                      <button
                        className="w-full py-2 rounded-lg font-semibold text-sm transition shadow bg-gray-300 text-gray-400 cursor-not-allowed"
                        disabled
                      >
                        Out of Stock
                      </button>
                      ...
                      <>
                        ...
                        <button
                          className="w-full py-2 rounded-lg font-semibold text-sm transition shadow bg-gradient-to-r from-blue-600 to-purple-600 text-white hover:opacity-90"
