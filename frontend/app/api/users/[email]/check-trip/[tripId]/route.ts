...
export async function GET(
  req: NextRequest,
  { params }: { params: Promise<{ email: string; tripId: string }> }
) {
  ...
  try {
    const { email, tripId } = await params;
    ...
    if (findResponse.ok && findResult.records && findResult.records.length > 0) {
      ...
      const userTrips = userRecord.fields?.trips || [];
      
      // Check if the tripId exists in the user's trips array
      const isRegistered = Array.isArray(userTrips) && userTrips.includes(tripId);
